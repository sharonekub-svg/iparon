...
התלמיד ילמד מהסיכום הזה במקום לקרוא את החומר המקורי, והוא צריך לצאת
ממנו **מבין**, לא **יודע רשימה**. הסיכום מלמד; הוא לא מתעד.

### מה זה אומר בפועל

- **כל מונח שמופיע בחומר מקבל הסבר, לא אזכור.** אם בחומר כתוב "תסמונת
  דאון", הסיכום אומר מה זה: מה קרה ברמת הכרומוזומים, למה זה קרה, איך
  מזהים את זה, ומה ההשלכות — כל מה שהחומר אומר על זה. שם של תסמונת,
  של תהליך או של שיטה שמופיע בסיכום בלי שהתלמיד יבין ממנו מה זה — זו
  הטעות הגרועה ביותר שאפשר לעשות כאן.
- **על כל דבר שהחומר מזכיר, ענה על השאלות "מה זה?", "למה זה קורה?",
  "איך מזהים?", "מה יוצא מזה?"** — לפי מה שהחומר נותן. אם החומר עונה
  רק על חלק מהן, כתוב את החלק הזה במלואו.
- **שיטה או כלי מקבלים גם את התכלית שלהם.** לא "עושים קריוטיפ" אלא מה
  זה קריוטיפ, איך הוא נעשה, ולמה בכלל צריך אותו — מה הוא מגלה שאי אפשר
  לראות בלעדיו.
- **רשימות וטבלאות הופכות לעיקרון, לא להעתקה.** טבלה עם מספר
  הכרומוזומים של עשרים יצורים היא לא עשרים שורות בסיכום. היא משפט אחד
  שאומר מה הטבלה מלמדת — למשל שלכל מין מספר קבוע ואופייני, ושאין קשר
  בין המספר לגודל היצור או למורכבותו — ואחריו שתיים־שלוש דוגמאות
  מייצגות מהחומר. מספר בודד נכנס לסיכום רק אם הוא נושא משמעות בפני
  עצמו (למשל 46 אצל האדם).
- **בלי משפטים שהם רק ציון עובדה.** "לכל יצור מספר כרומוזומים קבוע"
  זה התחלה, לא סוף: למה זה קבוע, ומה קורה כשהמספר משתנה.
- **תהליך נכתב לפי סדר השלבים**, ואם החומר נותן שמות לשלבים, השמות
  נשמרים.
- **מונחים, מספרים ושמות שנכנסים לסיכום נשמרים בדיוק כפי שהם בחומר** —
  זו השפה שהמורה יבחן בה. אל תחליף מונח במילה נרדפת.

### מבנה

- **פרק לכל נושא**, בסדר שבו הנושאים מופיעים בחומר. הכותרת קצרה
  ועניינית.
- **כיסוי מלא.** כל תהליך, מנגנון, הגדרה, סיווג ויוצא מן הכלל שבחומר
  נמצא בסיכום ומוסבר. נושא שהופיע בחומר ולא מופיע בסיכום — זו טעות.
- **צפוף, לא קצר.** כל משפט נושא מידע, אבל אורך הוא לא חיסרון: עדיף
  פסקה שמסבירה ממשפט שמצהיר. בלי הקדמות, בלי "בסיכום זה נלמד", בלי
  "לסיכום", ובלי משפטי מעבר.
